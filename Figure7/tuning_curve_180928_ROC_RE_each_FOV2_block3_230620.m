...
%}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function tuning_curve_180928_ROC_RE_each_FOV2_block3_230620
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
length_plot = min(length(use_neuron), 20);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    function make_ROC_box2(temp_ROC, temp_ROC_block, temp_ROC_choice, temp_ROC_sound)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    hige = 0.03;

    %ROC
    %Fill each shuffle box
    plot([0 1],[0 1],'k:','linewidth',0.5)
    hold on
    temp_x = [temp_ROC_block(1,1),temp_ROC_block(2,1),temp_ROC_block(2,1),temp_ROC_block(1,1)];
    temp_y = [temp_ROC_block(1,2),temp_ROC_block(1,2),temp_ROC_block(2,2),temp_ROC_block(2,2)];
    fill(temp_x,temp_y,[0 0 0]./255,'edgecolor','none')
    alpha(0.3)
    hold on
    %Fill each shuffle box
    %temp_x = [temp_ROC_choice(1,1),temp_ROC_choice(2,1)];
    temp_y = [temp_ROC_choice(1,2),temp_ROC_choice(1,2),temp_ROC_choice(2,2),temp_ROC_choice(2,2)];
    %line(temp_x,temp_y,'color',[105 105 105]./255,'linewidth',1)
    %line(temp_x,temp_y,'color',[1 0 0],'linewidth',1)
    fill([0 1 1 0],temp_y,[1 0 0],'edgecolor','none')
    alpha(0.3)
    hold on
    %Fill each shuffle box
    temp_x = [temp_ROC_sound(1,1),temp_ROC_sound(2,1),temp_ROC_sound(2,1),temp_ROC_sound(1,1)];
    %temp_y = [temp_ROC_sound(1,2),temp_ROC_sound(2,2)];
    %line(temp_x,temp_y,'color',[105 105 105]./255,'linewidth',1)
    fill(temp_x,[0 0 1 1],[0 0 1],'edgecolor','none')
    alpha(0.3)
    hold on
    %ROC value
    plot([temp_ROC(1),temp_ROC(1)],[temp_ROC(2)-hige,temp_ROC(2)+hige],'k','linewidth',1);
    hold on
    plot([temp_ROC(1)-hige,temp_ROC(1)+hige],[temp_ROC(2),temp_ROC(2)],'k','linewidth',1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    function make_ROC_box(temp_ROC, temp_ROC_block, temp_ROC_choice, temp_ROC_sound)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    %ROC
    plot(temp_ROC(1),temp_ROC(2),'k.');
    hold on
    %Fill each shuffle box
    temp_x = [temp_ROC_block(1,1),temp_ROC_block(2,1),temp_ROC_block(2,1),temp_ROC_block(1,1)];
    temp_y = [temp_ROC_block(1,2),temp_ROC_block(1,2),temp_ROC_block(2,2),temp_ROC_block(2,2)];
    fill(temp_x,temp_y,[180 180 180]./255,'edgecolor','none')
    hold on
    %Fill each shuffle box
    temp_x = [temp_ROC_choice(1,1),temp_ROC_choice(2,1)];
    temp_y = [temp_ROC_choice(1,2),temp_ROC_choice(2,2)];
    %line(temp_x,temp_y,'color',[105 105 105]./255,'linewidth',1)
    line(temp_x,temp_y,'color',[1 0 0],'linewidth',1)
    hold on
    %Fill each shuffle box
    temp_x = [temp_ROC_sound(1,1),temp_ROC_sound(2,1)];
    temp_y = [temp_ROC_sound(1,2),temp_ROC_sound(2,2)];
    %line(temp_x,temp_y,'color',[105 105 105]./255,'linewidth',1)
    line(temp_x,temp_y,'color',[0 0 1],'linewidth',1)
    set(gca,'xlim',[0 1],'ylim',[0 1])
    return
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [p_ROC,dif_ROC] = get_p_value_shuffle(temp_ROC, temp_shuf_block, jitter)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    shuffle_trials = length(temp_shuf_block);
        
    temp1 = 1 - length(find(temp_shuf_block(:,1) > temp_ROC(1)+jitter)) ./ shuffle_trials;
    temp2 = 1 - length(find(temp_shuf_block(:,1) < temp_ROC(1)-jitter)) ./ shuffle_trials;
    p_ROC(1) = min(temp1,temp2);

    temp1 = 1 - length(find(temp_shuf_block(:,2) > temp_ROC(2)+jitter)) ./ shuffle_trials;
    temp2 = 1 - length(find(temp_shuf_block(:,2) < temp_ROC(2)-jitter)) ./ shuffle_trials;
    p_ROC(2) = min(temp1,temp2);
    
    dif1 = temp_shuf_block(:,1) - temp_ROC(1);
    dif2 = temp_shuf_block(:,2) - temp_ROC(2);
    
    dif_ROC = [max(dif1),max(dif2)];
    
    return
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plot_reward_error_category(reward_category, error_category, use_color, category1, category2, use_std)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %plot_mean_se_moto(reward_category(category1).matrix,use_color(6,:),use_std)
    plot_mean_se_moto(reward_category(category1).matrix,[1 0 0],use_std)
    hold on
    %plot_mean_se_moto_dot(error_category(category1).matrix,use_color(6,:),use_std)
    plot_mean_se_moto_dot(error_category(category1).matrix,[1 0 0],use_std)
    hold on
    %plot_mean_se_moto_dot(reward_category(category2).matrix,use_color(1,:),use_std)
    plot_mean_se_moto_dot(reward_category(category2).matrix,[0 0 1],use_std)
    hold on
    %plot_mean_se_moto(error_category(category2).matrix,use_color(1,:),use_std)
    plot_mean_se_moto(error_category(category2).matrix,[0 0 1],use_std)
    set(gca,'xlim',[1 225],'xtick',[0:45:225])
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [sig_sound, sig_action] = sound_action_separate(ROC_sound, p_ROC_sound)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%If R activity is perfectly high than L, ROC is 1 

thre = 0.05;
%thre = 1;

sig_sound  = zeros(length(ROC_sound),1);
sig_action = zeros(length(ROC_sound),1);

%Get sig neurons
p_error = find(p_ROC_sound(:,1) < thre);
p_reward = find(p_ROC_sound(:,2) < thre);
sig_neuron = intersect(p_reward,p_error);

%Reward trials
temp_left = find(ROC_sound(:,2) < 0.5); %left trials
temp_right  = find(ROC_sound(:,2) >= 0.5); %right

ROC_sound = ROC_sound - 0.5;
ROC_sound = ROC_sound(:,1) .* ROC_sound(:,2);

temp_sound  = find(ROC_sound > 0); %sound
temp_sound   = intersect(sig_neuron,temp_sound);
temp_sound_L = intersect(temp_sound,temp_left);
temp_sound_R = intersect(temp_sound,temp_right);

temp_action = find(ROC_sound < 0); %action
temp_action = intersect(sig_neuron,temp_action);
temp_action_L = intersect(temp_action,temp_left);
temp_action_R = intersect(temp_action,temp_right);

sig_sound(temp_sound_L) = 1;
sig_sound(temp_sound_R) = 2;
sig_action(temp_action_L) = 1;
sig_action(temp_action_R) = 2;

return

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [activity_L, activity_R, LL_activity, LR_activity, RL_activity, RR_activity, L_category, R_category] = ...
          get_sound_trace_block(Sound_trace,Block_L,Block_R,Sound_L,Sound_R,Sound_category)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
