...
function tuning_curve_190925_decoder_single_neuron_100ms_only_ver230601
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
pathname3 = strcat('e:/Tone_discri1/all_mouse/single_decode/separate4/single_190925/100ms_only/');
cd(pathname3)
filename3 = dir('*.mat');
...
% sabun_correct_rate = all_correct_evi - all_correct;
% 
% hist_count_dif_neuron_group(sabun_correct_rate, all_sig_sound_or, sig_kruskal_sound, sound_neuron_modulated, sound_neuron_non_modulated, hist_x_sabun)
% 
% [median(all_correct(sound_neuron_modulated)), median(all_correct(sound_neuron_non_modulated))]
% ranksum(all_correct(sound_neuron_modulated), all_correct(sound_neuron_non_modulated))
% 
% [median(sabun_correct_rate(sound_neuron_modulated)), median(sabun_correct_rate(sound_neuron_non_modulated))]
% ranksum(sabun_correct_rate(sound_neuron_modulated), sabun_correct_rate(sound_neuron_non_modulated))
% 
...
correct_rate_box = [all_correct(all_sig_sound_or); all_correct(sound_neuron_modulated); all_correct(sound_neuron_non_modulated)];
label_box = [zeros(length(all_sig_sound_or),1); ones(length(sound_neuron_modulated),1); ones(length(sound_neuron_non_modulated),1).*2];
...
subplot(1,2,2)
boxplot(correct_rate_box, label_box)
